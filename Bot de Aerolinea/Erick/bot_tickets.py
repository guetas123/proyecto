...
token = '582285898:AAGDysb3KyuWkP-KehRTd6Tif5SGZiraJv4' #Token del Bot
    ...
    while continue1:
        ...
        if len(all_updates) > 0:
            for current_update in all_updates:
                ...
                #cuando inicie la conversacion, el /start se tomara como hola
                if first_chat_text == "/start":
                    first_chat_text = "hola"

                """
                #en este if se confirma que el usuario no haya escrito END, de ser asi, se termina el programa
                if first_chat_text != "termina":
                    response = response_module.process_message(first_chat_text)
                    erick.send_message(first_chat_id, response)
                    new_offset = first_update_id + 1
                else:
                    erick.send_message(first_chat_id, "Gracias. Nos vemos pronto")
                    new_offset = first_update_id + 1
                    continue1 = False
                """
                #el mensaje se procesa para definir y mandar una respuesta
                response = response_module.process_message(first_chat_text)
                erick.send_message(first_chat_id, response)
                new_offset = first_update_id + 1
